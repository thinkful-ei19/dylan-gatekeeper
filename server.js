...
function gateKeeper(req, res, next) {
  ...
  // const unparsedHeader = req.get('x-username-and-password');
  const parsedHeader = queryString.parse(req.get('x-username-and-password'));  
  const foundUser = USERS.find(user => user.userName === parsedHeader.user && user.password === parsedHeader.pass);
  foundUser ? req.user = foundUser : req.user = null;
